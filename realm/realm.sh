...
download() {
    ...
    local temp_file
    temp_file="$(mktemp)"
    if ! curl -sS -H "Accept: application/vnd.github.v3+json" -o "$temp_file" 'https://api.github.com/repos/zhboner/realm/releases/latest'; then
        "rm" "$temp_file"
    ...
    curl -L "https://github.com/zhboner/realm/releases/download/$version/$package" -o "$TEMPD/$package"
